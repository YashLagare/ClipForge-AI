...
//                     Create New Ad<Sparkles className="size-4 mr-2" />
//                   </GhostButton>
//                 </div>
//               </div>
//             </motion.div>
...
// import { AnimatePresence, motion } from 'framer-motion';
// import { Eye, FolderOpen, Grid, List, Loader2Icon, Search, Sparkles, Zap } from "lucide-react";
// import { useEffect, useState } from "react";
// import { Link, useNavigate } from "react-router-dom";
// import { dummyGenerations } from "../assets/assets";
// import { GhostButton, PrimaryButton } from "../components/Buttons";
...
//     { id: "all", label: "All Projects", icon: <FolderOpen className="size-4" /> },
//     { id: "published", label: "Published", icon: <Eye className="size-4" /> },
//     { id: "draft", label: "Unpublished", icon: <FileText className="size-4" /> },
//   ];
...
//     setFilteredGenerations(filtered);
//   }, [searchQuery, activeFilter, generations]);
...
const MyGeneration = () => {
  ...
  const fetchMyGenerations = async () => {
    setTimeout(() => {
      const generationsWithStats = dummyGenerations.map(gen => ({
        ...
        views: Math.floor(Math.random() * 5000) + 1000,
        shares: Math.floor(Math.random() * 200) + 10,
        isPublished: Math.random() > 0.3 ? true : false,
      }));
      setGenerations(generationsWithStats);
      setFilteredGenerations(generationsWithStats);
      setLoading(false);
  ...
  useEffect(() => {
    ...
    if (activeFilter !== "all") {
      if (activeFilter === "published") {
        filtered = filtered.filter(gen => gen.isPublished === true);
      } else if (activeFilter === "draft") {
        filtered = filtered.filter(gen => gen.isPublished === false);
      }
    }

    // Calculate stats
    stats.total = generations.length;
    stats.published = generations.filter(g => g.isPublished === true).length;
    stats.videos = generations.filter(g => g.generatedVideo).length;
    stats.images = generations.filter(g => !g.generatedVideo && g.generatedImage).length;
    stats.totalViews = generations.reduce((sum, g) => sum + ((g as any).views || 0), 0);
    stats.totalLikes = generations.reduce((sum, g) => sum + ((g as any).likes || 0), 0);
